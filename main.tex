...
尤其是硼和过渡金属原子形成的二维化合物会有许多新奇的性质。这使得二维硼化合物不但具有应用上的潜力，同时也可以作为合适候选体系以研究新的物理现象。
...
在周期性体系晶格的格点位置放置不同的元素，从原始晶格出发拓展超晶格并在格点放置不同元素能够拓展出各种各样的衍生结构，
而这些衍生结构中存在大量的等价构型。
构型的总数随着超胞体积的扩大急剧增加，因此通过结构上，简单的两两之间的几何对比很难对数量如此巨大的构型集合进行唯一性的挑选。
我们通过使用厄米标准型矩阵(HNF)来对表示衍生的晶格，并通过对晶格作用对称操作后进行对比查重，来得到确定体积的超晶格中物理不等价的晶格。针对固定的衍生晶格，我们对晶格位点进行了编号，后续晶体所属空间群的对称操作能够等价于一个对编号的置换操作。同时合并使用对不同的衍生的超晶格的HNF进一步划分归类，将具有相同的史密斯标准型矩阵(SNF)的超晶格归为同类。同类超晶格可以有相同的商群，因而有相同的不等价的标号顺序。因为SNF矩阵的数量要远远少于HNF矩阵的数量，通过这个方法，我们能够快速的对一系列具有相同SNF矩阵的晶格排除平移操作，有效提高了查重效率。
我们通过自己编写的软件实现了对这类衍生结构的查询，并向用户提供了友好的命令行以及网页端的接口来完成查重任务。

大型计算机快速增长的计算能力和不断发展的计算方法在计算材料学的发展中发挥了重大作用。
但是，这些发展同样带来了对海量数据计算和管理的新的挑战。
...
本人负责对其引擎的老旧的并发库依赖进行了更新，全面用库\texttt{asyncio}替换并移除了AiiDA对第三方并发库\texttt{tornado}的依赖，使其在插件等开发过程中能够使用最新的软件依赖。

针对二维硼材料结构上的共性，我们认为使用加入结构相关的先验知识进行有偏向性的筛选和搜索(biased-screening)的效果会优于使用全局搜索算法的无偏向性的结构搜索（unbiased-screening）。考虑到硼二维材料中硼原子之间倾向于保持包含空位的平面三角网格结构，我们使用结构查重软件优先构建相对合理的初始构型，使其尽可能均匀分布于势能面空间中目标结构能量局域极小值点附近的初始构型。再对所有的生成的结构进行第一性原理计算获得准确的能量和性质。
使用结构生成查重工具并合并使用高通量工作流工具AiiDA，我们对单层钛硼结构进行了结构稳定性和超导性质的理论研究。
通过该研究，我们不但确认了各个不同配比下已经报道过的几个钛硼单层材料，还发现了能量更低且具有优秀超导性质的\ce{TiB7}单层构型。需要强调的是，通过加入先验的结构信息，我们所生成和计算的结构总数相比全局搜索算法所涉及的结构数量大大减少，有偏的结构搜索方法逻辑简单，更容易结合工作流工具进行计算的管理，并且还获得了更好的结果。因此，我们认为对于特定材料，如果其结构上拥有固定特征，可以考虑使用该方法首先生成合理构型，来快速的筛选符合指定目标的结构。
